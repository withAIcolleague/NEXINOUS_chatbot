...
`src/middleware.ts`에서 모든 요청을 IP 화이트리스트로 필터링.

### 허용 IP 관리 (`.env.local`)

```bash
# 쉼표로 구분, 여러 IP 추가 가능
ALLOWED_IPS=127.0.0.1,::1,123.456.789.0
```

> - `127.0.0.1`, `::1` (로컬호스트)는 코드에서 **항상 허용**  
> - `.env.local`은 git에 커밋되지 않으므로 IP 노출 위험 없음

### 응답 방식

| 요청 종류 | 차단 응답 |
|---|---|
| 페이지 (`/`, `/chat` 등) | 403 HTML 안내 페이지 |
| API (`/api/*`) | `{ error: "접근이 거부되었습니다." }` JSON |

### 테스트 종료 후 제거 방법

`src/middleware.ts` 파일을 삭제하거나 `middleware` 함수에서 차단 로직을 제거.
